				...
				for (int k = 0; k < 3; k++)
				{
					for (int h = i - kernel_size/2 ; h <= i + kernel_size/2 ; h++)
					{
						for (int w = j - kernel_size/2; w <= j + kernel_size/2 ; w++)
						{
							...
							int y = w;
							if (h >= output_height) {
								x = output_height - (h - output_height + 1);
							} else if (h < 0) {
								x = 0 - h;
							}
							if (w >= output_width) {
								y = output_width - (w - output_width + 1);
							} else if (w < 0) {
								y = 0 - w;
							}
							outputImage.at<Vec3b>(i,j)[k] += 
								kernel[h-i+(kernel_size/2)][w-j+(kernel_size/2)] * 
								image.at<Vec3b>(x,y)[k];
						}
					}
				}	
			}
		}
		octaves.push_back(outputImage);                                          
	    free_kernel_mem(kernel, kernel_size);
	}
				...
				for (int level = -1; level < 2; level++) {
					...
					for (int x = -1; x < 2; x++) {
						...
						for (int y = -1; y < 2; y++) {
							...
							if (!isGreatest && !isLeast) {
								break;
							} else if (level == 1 && x == 1 && y == 1 && curr_octave == 1) {
